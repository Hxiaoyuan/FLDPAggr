...
from utils.data_partition import create_dirichlet_distribution, split_by_distribution
...
def get_dataset(dataset='MNIST', num_users=20, unequal=False, user_max_class=10, alpha=10, seed=0):
    ...
    if dataset == 'CIFAR10':
        ...
        #         # Chose euqal splits for every user
        #         user_groups = noniid(train_dataset, num_users, user_max_class)
        distribution = create_dirichlet_distribution(alpha, num_users, num_class, seed)
        train_split = split_by_distribution(np.array(train_dataset.targets), distribution)
        user_groups = train_split
    return train_dataset, test_dataset, user_groups



def get_dataset_CIFAR(dataset='CIFAR10', num_users=20, iidness=True, unequal=False, user_max_class=10, alpha=10, seed=0):
    """ Returns train and test datasets and a user group which is a dict where
    the keys are the user index and the values are the corresponding data for
    each of those users.
    """
