...
def models_to_vertical_matrix(models):
    client_vertical_mat = {}
    for i, model in enumerate(models):
        client_vertical_mat[i] = {}
        for n, p in model.named_parameters():
            p1 = torch.nn.utils.parameters_to_vector(p)
            client_vertical_mat[i][n] = p1.cpu().detach().numpy()
    return client_vertical_mat
...
def load_matrix_denoising_module_to_models(matrix_denoising, new_models, old_models):

    for idx, old_model in enumerate(old_models):
        # old_model_parm = model_to_params(old_model)
        with torch.no_grad():
            for name, param1 in old_model.named_parameters():
                for name1, param2 in new_models[idx].named_parameters():
                    if name1 == name:
                        param2.data = deepcopy(param1.data.cpu()) + matrix_denoising[idx][name].reshape(deepcopy(param1.data).shape)
    return
...
# def find_delta_module(new_model, old_model):
#     delta_model = zero_model(new_model)
#     for i, param in enumerate(delta_model.parameters()):
#         new_model_param = model_to_params(new_model)[i]
#         old_model_param = model_to_params(old_model)[i]
