...
import torch
from utils.eval import accuracy, accuracies, losses, precision_recall_f1
...
def individual_train_PDP(train_loader, loss_func, optimizer, model, privacy_engine, delta, test_loader, device, \
    ...
    if save:
        ...
        if not os.path.exists(csv_file):
            to_csv(csv_file, ['epoch', 'loss', 'test acc', 'test precision', 'test recall', 'test f1'], mode='a')
    ...
    acc = accuracy(model, test_loader, device, show=show)
    pre, rec, f1 = precision_recall_f1(model, test_loader, device, show=show)
    if save:
        to_csv(csv_file, [epoch, loss.item(), acc, pre, rec, f1], mode='a')
    if save:
        torch.save({'epoch': epoch, 'state_dict': model.state_dict(), \
                'optimizer' : optimizer.state_dict(),}, output_dir + f'/model_{client_id}_last.pth')
